"""
    ...
    aSi and alpha parameters are initialized from previous best Powell optimization algorithm results.
...
from utils_optimization.results_handling import save_results,load_best_run
...
# Acquiring results from our best run of Powell algorithm
powell_dir='/Users/jaworskj/DataspellProjects/General_purpose_hyperspectral_thermal_camera/Lorentzian_function_approach/sweep_optimization_1_4/sweep_powell_results'
best_run=load_best_run(powell_dir)
silicon_parameters=best_run[0][(n-1)*4:-1].detach().numpy()
alpha_parameter=best_run[0][-1].detach().numpy()
...
for k in tqdm(range(5)):
    results_dir = os.path.join(os.getcwd(), f"optimization_sweep_powell_modified")
    ...
    for i in range((n - 1) * 2):
        params.add(f'aSi_filter_{i + 1}', value=silicon_parameters[i], min=1000e-9, max=10000e-9)

    # Bandwidth of Lorentzian
    params.add(f'Sweep_range', value=alpha_parameter, min=1, max=4)
