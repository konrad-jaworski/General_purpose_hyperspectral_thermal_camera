...
def save_results(params, residuals, run_id):
    ...
    try:
        ...
        torch.save(lmfit_to_torch_values(params), param_file)
...
for j in range(500):
    results_dir = os.path.join(os.getcwd(), "optimization_runs")
    ...
    save_success = save_results(result.params, Residuals_over, run_id)
    if not save_success:
...
#torch_optimal=lmfit_to_torch_values(result.params)
#torch.save(torch_optimal,'optimal_all_tr.pt')
